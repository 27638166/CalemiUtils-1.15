        ...
        if (isTextLayer || isHotbarLayer) {
            ...
            if (!player.isSpectator()) {
                ...
                if (CUConfig.wallet.walletOverlay.get() && !walletStack.isEmpty() && mc.currentScreen == null) {
                    ...
                    if (isTextLayer) Minecraft.getInstance().fontRenderer.drawString(currencyStr, 21 + xOffsetStr, 4 + yOffset, 0xFFFFFFFF);
                    //Draw Item on the HOTBAR layer.
                    else if (isHotbarLayer) ScreenHelper.drawItemStack(Minecraft.getInstance().getItemRenderer(), walletStack, 2 + xOffsetItem, yOffset);
                }
            }
        }
    }
