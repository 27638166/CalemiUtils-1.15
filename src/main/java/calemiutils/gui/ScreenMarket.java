...
public class ScreenMarket extends GuiScreenBase {
    ...
    public void drawGuiBackground(int mouseX, int mouseY) {
        ...
        //Renders title.
        ScreenHelper.drawCenteredString("Market", getScreenX(), getScreenY() - 71, 100, 0xFFFFFF);
        ...
        if (marketItem != null && !selectedStack.isEmpty()) {
            ...
            ScreenHelper.drawCenteredString(name, x, yOffset + 18, 0, 0xFFFFFF);

            ScreenHelper.drawCenteredString((market.buyMode ? "Cost " : "Sell ") + TextFormatting.GOLD + StringHelper.printCurrency(marketItem.value * market.purchaseAmount), x, yOffset + 32, 0, 0xFFFFFF);

            ScreenHelper.drawCenteredString(market.purchaseAmount + "x", x, yOffset + 46, 0, 0xFFFFFF);
    ...
    public boolean mouseClicked(double x, double y, int i) {
        ...
        if (!market.automationMode) {
            ...
            if (buyTab.getRect().contains((int) x, (int) y)) {
                ...
                setBuyMode(true);
                if (buyTab.getMarketButtons().size() > 0)
                    setSelectedIndex(buyTab.getMarketButtons().get(0).getMarketListIndex());
            }

            if (sellTab.getRect().contains((int) x, (int) y)) {
                minecraft.getSoundHandler().play(SimpleSound.master(SoundEvents.UI_BUTTON_CLICK, 1));
                setBuyMode(false);
                if (sellTab.getMarketButtons().size() > 0)
                    setSelectedIndex(sellTab.getMarketButtons().get(0).getMarketListIndex());
            }
        }
